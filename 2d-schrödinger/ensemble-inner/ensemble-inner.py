...
MODEL_PATH_ANN = "../ann-inner/model-2d-schrödinger-ann.pt"
...
MODEL_PATH_GCN = "../gcn-inner/model-2d-schrödinger-gcn.pt"
...
def test_model(true_re,true_im):
    ...
    test_loss_u = loss_func(u_pred[te_samp], true_re[te_samp])
    test_loss_v = loss_func(v_pred[te_samp], true_im[te_samp])

    test_loss = test_loss_u + test_loss_v      
    print('test loss:', test_loss.item())

    psi_pred = torch.sqrt(u_pred[te_samp]**2 + v_pred[te_samp]**2)
    psi = torch.sqrt(true_re[te_samp]**2 + true_im[te_samp]**2)
