...
class ANN_Model(nn.Module):
    ...
    def __init__(self, input_feat, hidden, output_feat):
        ...
        self.dense5 = nn.Linear(hidden,output_feat)
...
class Ensemble(nn.Module):
    ...
    def forward(self, x):
        ...
        out_u = torch.cat((torch.unsqueeze(out1[:,0],1), torch.unsqueeze(out2[:,0],1)), dim=1) 
        out_v = torch.cat((torch.unsqueeze(out1[:,1],1), torch.unsqueeze(out2[:,1],1)), dim=1)    

        out_u = self.fc1(out_u)
        out_u = F.tanh(out_u)
        out_u = self.fc2(out_u)

        out_v = self.fc1(out_v)
        out_v = F.tanh(out_v)
        out_v = self.fc2(out_v)    
